...
router.get('/:orderId', async (req, res) => {
  ...
  if (expand === 'products') {
    const products = await Promise.all(order.products.map(async (prod) => {
      const productDetails = await Product.findByPk(prod.productId);
      return { ...prod, product: productDetails };
    }));
